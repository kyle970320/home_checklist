...
import { CheckCheck, ChevronLeft, CircleAlert, FileText, House, ShieldCheck } from 'lucide-react'
import { Link, useParams } from 'react-router-dom'
import ThemeToggle from '../components/ThemeToggle'
import { getChecklist, toggleChecklistItem, updateChecklistCalculator, updateChecklistChoice } from '../data/checklists'
import type { Checklist, ChecklistCalculator, ChecklistChoice, ChecklistItem } from '../types/checklist'

function countChecklistProgress(sections: Checklist['sections']) {
  const items = sections.flatMap((section) => section.groups.flatMap((group) => group.items))
  const checked = items.filter((item) => item.checked || item.choice?.value !== null).length
...
function calculateDepositRatio(calculator: ChecklistCalculator) {
  ...
  if (!marketPriceInput || marketPriceInput <= 0 || depositInput === null || maxDebtInput === null || safePercent === null) {
    return null
  }
...
function CalculatorCard({
  ...
  const handleCalculatorChange =
    (field: keyof ChecklistCalculator) => (event: React.ChangeEvent<HTMLInputElement>) => {
      const next = updateChecklistCalculator(checklistId, item.id, { [field]: event.target.value })
    ...
    <div className="mt-3 rounded-2xl border border-indigo-200 bg-indigo-50/80 p-3 dark:border-indigo-400/20 dark:bg-indigo-500/10">
      <div className="grid gap-3 xl:grid-cols-2">
        <label className="grid gap-1.5">
          <span className="text-xs font-semibold text-zinc-600 dark:text-white/70">주택 시세 (억)</span>
        ...
        <label className="grid gap-1.5">
          <span className="text-xs font-semibold text-zinc-600 dark:text-white/70">보증금 (천)</span>
          <div className="flex overflow-hidden rounded-xl border border-zinc-200 bg-white focus-within:border-indigo-400 dark:border-white/10 dark:bg-zinc-950/60">
            <input
        ...
        <label className="grid gap-1.5">
          <span className="text-xs font-semibold text-zinc-600 dark:text-white/70">채권최고액 (천)</span>
          <div className="flex overflow-hidden rounded-xl border border-zinc-200 bg-white focus-within:border-indigo-400 dark:border-white/10 dark:bg-zinc-950/60">
            <input
        ...
        <label className="grid gap-1.5">
          <span className="text-xs font-semibold text-zinc-600 dark:text-white/70">안전 기준 %</span>
          <div className="flex overflow-hidden rounded-xl border border-zinc-200 bg-white focus-within:border-indigo-400 dark:border-white/10 dark:bg-zinc-950/60">
            <input
      ...
      {result ? (
        <div className="mt-3 grid gap-2 sm:grid-cols-3">
          <div className="rounded-xl border border-zinc-200 bg-white px-3 py-3 dark:border-white/10 dark:bg-zinc-950/60">
            <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-500 dark:text-white/50">합계</div>
            <div className="mt-1 text-sm font-bold">{formatNumber(result.total)}원</div>
          </div>
          <div className="rounded-xl border border-zinc-200 bg-white px-3 py-3 dark:border-white/10 dark:bg-zinc-950/60">
            <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-500 dark:text-white/50">계산 비율</div>
            <div className="mt-1 text-sm font-bold">{result.ratio.toFixed(1)}%</div>
          </div>
          <div
          ...
          >
            <div className="text-[11px] font-semibold uppercase tracking-[0.14em] opacity-80">안전 여부</div>
            <div className="mt-1 text-sm font-bold">
              {result.isSafe ? `기준 이하 (${result.safePercent}%)` : `기준 초과 (${result.safePercent}%)`}
            </div>
          </div>
        </div>
      ) : (
        <p className="mt-3 text-xs leading-5 text-zinc-500 dark:text-white/55">
          시세는 억원, 보증금과 채권최고액은 천만원 단위로 입력하면 자동으로 비율을 계산해 안전 기준 이하인지 보여줍니다.
        </p>
      )}
    </div>
  )
...
export default function DetailPage() {
  const { id } = useParams()
  const [item, setItem] = useState<Checklist | null>(() => (id ? getChecklist(id) : null))
    ...
    return (
      <div className="min-h-dvh bg-zinc-50 text-zinc-900 transition-colors dark:bg-zinc-950 dark:text-white">
        <div className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h1 className="text-2xl font-black sm:text-3xl">존재하지 않는 체크리스트</h1>
              <p className="mt-2 text-sm text-zinc-600 dark:text-white/70">ID: {id}</p>
            </div>
            <ThemeToggle />
          </div>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 sm:px-6 sm:py-8">
          ...
          <div className="bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.18),_transparent_34%),linear-gradient(135deg,_rgba(255,255,255,0.98),_rgba(244,244,245,0.92))] px-5 py-6 dark:bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.24),_transparent_34%),linear-gradient(135deg,_rgba(24,24,27,0.96),_rgba(9,9,11,0.92))] sm:px-7 sm:py-8">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                <p className="text-sm font-semibold uppercase tracking-[0.22em] text-indigo-500 dark:text-indigo-300">Monthly Rent</p>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-white/70">
                  등기부등본 확인부터 현장 점검, 계약서 특약까지 한 번에 체크할 수 있는 월세 체크리스트예요. 체크와 계산 값은
                  이 브라우저에 자동 저장됩니다.
                </p>
              </div>
              ...
              <div className="grid min-w-[250px] gap-3 sm:grid-cols-3 lg:min-w-[320px] lg:grid-cols-1">
                <div className="rounded-2xl border border-zinc-200/80 bg-white/80 p-4 backdrop-blur dark:border-white/10 dark:bg-white/6">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-white/45">진행률</div>
                  <div className="mt-2 flex items-end gap-2">
                    <span className="text-3xl font-black">{progress.percent}%</span>
                    <span className="pb-1 text-sm text-zinc-500 dark:text-white/50">{progress.checked}/{progress.total}</span>
                  </div>
                  <div className="mt-3 h-2 rounded-full bg-zinc-200 dark:bg-white/10">
                    <div className="h-full rounded-full bg-indigo-500 transition-[width]" style={{ width: `${progress.percent}%` }} />
                  </div>
                </div>

                <div className="rounded-2xl border border-zinc-200/80 bg-white/80 p-4 backdrop-blur dark:border-white/10 dark:bg-white/6">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-white/45">섹션 수</div>
                  <div className="mt-2 text-3xl font-black">{item.sections.length}</div>
                  <div className="mt-2 text-sm text-zinc-500 dark:text-white/50">등기부등본, 현장 확인, 특약사항</div>
                </div>

                <div className="rounded-2xl border border-zinc-200/80 bg-white/80 p-4 backdrop-blur dark:border-white/10 dark:bg-white/6">
                  <div className="text-xs font-semibold uppercase tracking-[0.18em] text-zinc-500 dark:text-white/45">생성일</div>
                  <div className="mt-2 text-lg font-bold">{formatDate(item.createdAt)}</div>
                  <div className="mt-2 text-sm text-zinc-500 dark:text-white/50">체크 상태와 계산 결과는 자동 저장됩니다.</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {item.sections.map((section) => {
            ...
            const doneCount = sectionItems.filter((entry) => entry.checked || entry.choice?.value !== null).length

            return (
              <article key={section.id} className={`rounded-[26px] border p-4 shadow-sm transition-colors sm:p-5 ${sectionTone(section.tone)}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {section.description ? <p className="mt-1 text-sm leading-6 text-zinc-600 dark:text-white/70">{section.description}</p> : null}
                    </div>
                  </div>
                ...
                <div className="mt-5 space-y-4">
                  {section.groups.map((group) => (
                    <div key={group.id} className="rounded-2xl border border-zinc-200/80 bg-white/80 p-4 dark:border-white/8 dark:bg-black/10">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <h3 className="text-sm font-black sm:text-base">{group.title}</h3>
                          {group.description ? <p className="mt-1 text-xs leading-5 text-zinc-500 dark:text-white/55">{group.description}</p> : null}
                        </div>
                        <div className="text-xs font-semibold text-zinc-500 dark:text-white/45">
                          {group.items.filter((entry) => entry.checked || entry.choice?.value !== null).length}/{group.items.length}
                        </div>
                      </div>
                      ...
                      <div className="mt-3 space-y-2.5">
                        {group.items.map((entry) => (
                          ...
                          >
                            {entry.choice ? (
                              <div>
                                <div className="text-sm font-semibold leading-6">{entry.label}</div>
                                {entry.hint ? <p className="mt-1 text-xs leading-5 text-zinc-500 dark:text-white/55">{entry.hint}</p> : null}
                                <ChoiceSwitch
                                  checklistId={item.id}
                                  item={entry}
                                  onChecklistChange={(next) => setItem((current) => updateLocalChecklist(current, next))}
                                />
                              </div>
                            ) : (
                              <>
                                <label className="flex cursor-pointer items-start gap-3">
                                  <input
                                    ...
                                    onChange={() => {
                                      setItem((current) => updateLocalChecklist(current, toggleChecklistItem(item.id, entry.id)))
                                    }}
                                  />
                                  <div className="min-w-0">
                                    <div className={`text-sm font-semibold leading-6 ${entry.checked ? 'text-zinc-400 line-through dark:text-white/35' : ''}`}>
                                      {entry.label}
                                    </div>
                                    {entry.hint ? <p className="mt-1 text-xs leading-5 text-zinc-500 dark:text-white/55">{entry.hint}</p> : null}
                                  </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-zinc-600 dark:text-white/70">
                위험 등기 여부, 보증금 비율, 누수와 곰팡이 흔적은 한 번 더 교차 확인하는 편이 좋아요. 계약 직전에는 특약사항 문구가 실제 계약서에 반영됐는지도 꼭 확인하세요.
              </p>
            </div>
          </div>
