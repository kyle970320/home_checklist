      ...
      <div className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {items.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`group overflow-hidden rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition-colors dark:border-white/10 dark:bg-white/[0.04] ${
                ...
                <div className="grid gap-3">
                  {isEditing ? (
                    ...
                    <>
                      ...
                      <div className="text-xs text-zinc-500 dark:text-white/50">카드 전체를 클릭하면 상세로 이동</div>
                    </>
                  )}
                </div>
              </div>
        ...
        >
          <p className="m-0 text-sm">
            <span className="font-extrabold">{deleteState.target?.title}</span> 체크리스트를 삭제합니다. 이 작업은 되돌릴 수 없어요.
          </p>
        </Modal>
      </div>
    </div>
